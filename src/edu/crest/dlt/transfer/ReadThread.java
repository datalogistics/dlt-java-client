...
public class ReadThread extends TransferThread
{
	...
	private int pushback_index(int try_count)
	{
		...
		synchronized (queued_read_jobs) {
			// return Math.min(queued_read_jobs.size(), (max_tries_allowed *
			// try_count) * retry_interval);
			return max_tries_allowed > try_count ? Math.min(queued_read_jobs.size(),
	...
	public Job job_next()
	{
		...
		synchronized (queued_read_jobs) {
			...
			// log.info(this + ": " + queued_read_jobs.size() +
			// " read-jobs pending.");
		}
		return read_job;
	}
		...
		while (!isInterrupted() && queued_read_jobs != null && are_jobs_pending()) {
			...
			/* add job to writer's queue, and consider done */
				...
				default:
					...
					log.info(this + " [EXECUTED] " + read_job.status() + "; pushed behind " + pushback_idx
							+ " jobs.");
					synchronized (queued_read_jobs) {
