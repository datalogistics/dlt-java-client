...
public class Status
{
	public enum ui_status {
		depot_connected,
		depot_not_connected,
		download_ready, upload_ready,
		metadata_error,
		mappings_inaccessible,
		not_enough_depots, file_not_found,
		download_failed,
		upload_failed,
		transfer_failed,
		transfer_sucess,
		transfer_aborting, transfer_aborted,
		processing,
		downloading,
		uploading,
		file_registering
  ...
  private static Map<ui_status, String> status_text = new HashMap<ui_status, String>() {{
  	...
  	put(ui_status.mappings_inaccessible, "One or more mapping(s) of the file not currently accessible.");
  	put(ui_status.not_enough_depots, "Not enough depot(s) accessible for hosting required copies of file.");
