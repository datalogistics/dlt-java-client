  ...
  private void initComponents() {
	...
	{// GEN-FIRST:event_input_depots_all_clicked
		...
		for (int row = 0; row < table_depots_model.getRowCount(); row++) {
			...
			log.info((Depot) table_depots_model.getValueAt(row, column_index_depot) + (input_depots_all.isSelected() ? " selected." : " deselected."));
		}
	...
	public void sync_depots()
	{
		...
		for (int row = 0; row < table_depots_model_default.getRowCount(); row++) {
			table_depots_model_default.removeRow(0);
			log.info((Depot) table_depots_model.getValueAt(row, column_index_depot) + " removed.");
		}
		
		/* repopulate depot-list */
		for (Depot depot : Depot.depots()) {
			table_depots_model_default.addRow(new Object[] { false, depot, (depot.connected() ? icon_depot_connected : icon_depot_not_connected) });
		}
	}
	...
	private static final Icon icon_depot_connected = new ImageIcon("res/GreenDot.gif");
	private static final Icon icon_depot_not_connected = new ImageIcon("res/RedDot.gif");
